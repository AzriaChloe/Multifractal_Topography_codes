function [lmcosi,dw]=xyz2plm_modified(fthph,L,method,option,lat,lon,cnd)
...
disp('decomposition sur la base de harmonique spherique')
% disp(L);
t0=clock;
...
if exist([myname(fnpl,L),'.mat'],'file')==2 && as==1 && option==0 
...
function result= remove_na(Plm)

result=zeros(size(Plm));
...
pos=find((col_select==Inf) | isnan(col_select) | col_select==0);

 

%1st zeros are ok

pos_first=length(find(cumsum(col_select)==0));

 

%corrected version

corrected=col_select;

 

%1rst segment

pos1=pos(1:round(length(pos)/2));

pos1=pos1(pos1>pos_first);

 

%2nd segment

pos2=pos(round(length(pos)/2)+1:end);

pos2=pos2(pos2<(length(col_select)-pos_first));

 

%position of 1st segment to duplicate

pos1new=pos1+length(pos1);

pos1new_bis=pos1-length(pos1);
...
pos1new_bis(pos1new_bis<=0)=1;

 

%std

std1=std(col_select(pos1new));

get_std=(std(col_select(pos1new_bis))+std(col_select(pos1new)))/2;

 

%position of 2nd segment to duplicate

pos2new=pos2-length(pos2);

pos2new(pos2new>length(col_select))=length(col_select);

 

%substitution

corrected(pos1)=col_select(pos1new)/std1*get_std;

corrected(pos2)=col_select(pos2new)/std1*get_std;
